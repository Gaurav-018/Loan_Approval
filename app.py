...
# 2. Open the compressed gzip archive asset directly
...
try:
    if os.path.exists(MODEL_PATH):
        ...
        # Fallback to standard pkl if you haven't compressed it yet
        ...
        if os.path.exists(MODEL_PATH):
            ...
            model = None
            print("Warning: No model files (model.pkl or model.pkl.gz) found.")
except Exception as e:
    print(f"Error parsing model archive: {str(e)}")
    model = None
...
def process_features(data: dict) -> list:
    """Structures raw input data directly into an array without using Pandas."""
...
def predict():
    if model is None:
        return jsonify({"error": "Model missing or loading broken."}), 500
...
if __name__ == "__main__":
    # Render fallback directly if run locally
